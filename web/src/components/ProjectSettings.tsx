...
import { X, Plus, Trash2, Eye, EyeOff, Save, Edit3, Server, Key, GitBranch, Loader } from 'lucide-react';
import {
...
export const ProjectSettings: React.FC<ProjectSettingsProps> = ({ sessionName, onClose }) => {
  ...
  const [flashSvcId, setFlashSvcId] = useState<number | null>(null);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (activeTab === 'variables') varKeyRef.current?.focus();
      else if (activeTab === 'services') svcNameRef.current?.focus();
    }, 50);
    return () => clearTimeout(timer);
  }, [activeTab, loading]);

  // Smart Escape: cancel edit first, then close modal
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (editingVarId !== null) { setEditingVarId(null); return; }
        if (editingSvcId !== null) { setEditingSvcId(null); return; }
        onClose();
      }
    };
    document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [onClose, editingVarId, editingSvcId]);
  ...
  const flashSvc = (id: number) => {
    setFlashSvcId(id);
    setTimeout(() => setFlashSvcId(null), 600);
  };

  // ---- Variable handlers ----
  const handleAddVar = async () => {
    if (!newVarKey.trim()) return;
    const result = await createProjectEnvVar(sessionName, newVarKey.trim(), newVarValue, newVarSecret);
  ...
  const handleAddService = async () => {
    ...
    const result = await createProjectService(sessionName, newSvcName.trim(), parseInt(newSvcBase), newSvcType, newSvcEnvKey.trim());
    setNewSvcName(''); setNewSvcBase(''); setNewSvcType('port'); setNewSvcEnvKey('');
    await loadAll();
    if (result?.id) flashSvc(result.id);
    svcNameRef.current?.focus();
  };
  ...
  const tabs: { id: SettingsTab; label: string; icon: React.ElementType; count: number }[] = [
    { id: 'variables', label: 'VARIABLES', icon: Key, count: vars.length },
    { id: 'services', label: 'SERVICES', icon: Server, count: services.length },
    { id: 'worktrees', label: 'WORKTREES', icon: GitBranch, count: slots.length },
  ];
          ...
          <div className="flex">
            {tabs.map(tab => (
              ...
              >
                ...
                {tab.label}
                {!loading && tab.count > 0 && (
                  <span className={`text-[9px] ml-0.5 ${activeTab === tab.id ? 'text-green-500' : 'text-green-800'}`}>
                    ({tab.count})
                  </span>
                )}
              </button>
  ...
  function renderServicesTab() {
    return (
      <div className="border border-green-900/50">
        ...
        {services.length === 0 ? (
          <div className="flex flex-col items-center py-8">
            <Server className="w-8 h-8 text-green-900 mb-2" />
          ...
          services.map(s => (
            <div
              key={s.id}
              className={`flex items-center px-3 py-2 border-b border-green-900/30 transition-all duration-500 ${
                flashSvcId === s.id ? 'bg-green-900/30' :
              ...
              {editingSvcId === s.id ? (
                <>
                  <input value={editSvcName} onChange={e => setEditSvcName(e.target.value)}
                    onKeyDown={e => editKeyHandler(e, () => handleUpdateService(s.id), () => setEditingSvcId(null))}
        ...
        {/* Add row — always rendered once */}
        <div className="flex flex-wrap items-center gap-2 px-3 py-2 bg-black/30">
          <input ref={svcNameRef} value={newSvcName} onChange={e => setNewSvcName(e.target.value)}
