...
#### Project Environment & Worktree Port Isolation

项目级环境变量和 Worktree 端口隔离系统。类似 GitHub Secrets & Variables，在 Web UI 中为每个项目配置环境变量和服务端口，创建 worktree 时自动分配隔离端口并生成 `.worktree.env`。

##### 数据模型

```sql
-- 项目自定义变量（完全自由的 key-value，类似 GitHub Repo Variables）
...
CREATE TABLE IF NOT EXISTS project_services (
    ...
    value_type   TEXT NOT NULL DEFAULT 'port',  -- 'port' | 'db_index'
...
```
slot = 下一个可用编号 (1~15，0 保留给 main)
service_value = base_value + slot

例: mediahub 定义 frontend(base=5175), backend(base=8080), redis_db(base=0)
    slot 2 → FRONTEND_PORT=5177, BACKEND_PORT=8082, REDIS_DB=2
```

##### .worktree.env 生成

创建 worktree 时自动生成，修改项目变量时自动同步更新所有 worktree：

```env
# Auto-generated by ai-tracker (do not edit)
# Project: mediahub | Branch: feature/payment | Slot: 2
...
创建 worktree 时自动往对应的 Claude Code MEMORY.md 写入指针（非数据）：

```
~/.claude/projects/-{path-encoded-worktree}/memory/MEMORY.md

内容: "When starting dev servers or configuring ports, read .worktree.env in the project root."
```

AI 运行时读取 `.worktree.env` 获取实际值，变量更新后无需修改 MEMORY.md。

##### API Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/project/env-vars?session_name=xx` | 获取项目环境变量列表 |
| POST | `/api/project/env-vars` | 创建环境变量 |
| PUT | `/api/project/env-vars/:id` | 更新环境变量（触发全量同步） |
| DELETE | `/api/project/env-vars/:id` | 删除环境变量（触发全量同步） |
| GET | `/api/project/services?session_name=xx` | 获取项目服务定义 |
| POST | `/api/project/services` | 创建服务定义 |
| PUT | `/api/project/services/:id` | 更新服务定义（触发全量同步） |
| DELETE | `/api/project/services/:id` | 删除服务定义（触发全量同步） |
| GET | `/api/project/worktree-slots?session_name=xx` | 获取 slot 分配列表 |
| POST | `/api/project/worktree-slots` | 分配 slot（自动选择下一个可用） |
| DELETE | `/api/project/worktree-slots/:id` | 释放 slot |

所有变量/服务的变更操作自动触发 `sync_all_worktree_envs(session_name)`，重写该项目所有 worktree 的 `.worktree.env`。
...
```
[Variables]  — GitHub 风格的 key-value 编辑器，secret 值掩码显示
[Services]   — 服务定义表格（service_name, base_value, value_type, env_key）
[Worktrees]  — Slot 分配总览，显示每个 worktree 的计算后端口
```

项目入口从 tmux session 升级为项目维度，侧边栏按项目名导航，点击项目进入管理页面，从项目页面打开/恢复 tmux session。
